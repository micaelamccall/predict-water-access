...
```{r}
library(car)
vifs<-bind_cols(as.data.frame(names(vif(model))), as.data.frame(unname(vif(model))));vifs
```

Since several of these variables have a large VIF, some should be removed. We can do this with the following code, that removes the variable with the largest VIF and re-builds the model until no variables have  VIF above 4. 

```{r}
while(any(vifs$`unname(vif(model))`>4)){  #while any VIFS are greater than 4
  ...
  model<-lm(formula, data = wb_transformed) #create a new model
...
```{r}
resid <- bind_cols(as.data.frame(fitted(model)), as.data.frame(residuals(model))) 

p1 <- ggplot(resid,aes(x= fitted(model), y=residuals(model))) + geom_point() + labs(title='Residual Plot', x='Fitted Values', y='Residuals');p3
...
```{r}
p2 <- ggplot(resid, aes(residuals(model))) + geom_histogram(bins=100) + labs(title = 'Histogram of Residuals')
# with the histogram, we're looking for a normal distribution
p3 <- ggplot(resid, aes(sample=residuals(model))) + geom_qq() + geom_qq_line() + labs(title = "Normal Q-Q Plot of Residuals") 
